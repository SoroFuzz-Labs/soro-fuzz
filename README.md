...
## Wire your own contract: the token walkthrough

The counter above is the smallest possible wiring: one `i64`, one inline
invariant. The token example is the path most real contracts follow — a
multi-balance state machine that **reuses** `soro-fuzz-invariants` instead of
hand-rolling every check.

Read `examples/token/src/harness.rs` alongside this list.

1. **Contract** (`examples/token/src/lib.rs`) — SEP-41-shaped `mint` / `burn` /
   `transfer` with an admin. Same `#![no_std]` + `testutils` pattern as the
   counter.

2. **Shadow model** (`TokenModel` in `harness.rs`) — tracks `admin`, a
   `Vec<(Address, i128)>` of balances, and `total_supply`. Unlike the counter's
   single `expected_count`, this model has to stay consistent across many
   addresses.

3. **Trait bridges into shared invariants** — instead of a bespoke
   `CounterValueMatchesModel`, the token implements the small adapter traits
   the reusable library needs:
   - `HasBalances` / `HasTotalSupply` → feed `NoNegativeBalance` and
     `SupplyConservation`
   - `RequiresAuthorizer` (on the command side) → feed
     `AuthRequiredForMutation`

   That is the whole point of this example: `soro-fuzz-core` and
   `soro-fuzz-invariants` needed **no changes** to support a second contract
   shape. The counter's inline invariant is fine for a toy; production
   contracts should prefer the shared ones (and only add inline checks for
   domain rules the library does not cover).

4. **Command enum** (`TokenCommand`) — `Mint` / `Burn` / `Transfer` with
   `AddressIndex` + `NonNegativeI128` so the fuzzer spends budget on
   interesting edges (insufficient balance, auth misses) rather than always
   bouncing off the trivial negative-amount rejection.

5. **`Command::execute`** — mock auth via `MockAuth` / `mock_single_auth`,
   call `try_mint` / `try_burn` / `try_transfer`, and classify with
   `Outcome::from_try_result` (never hand-match the nested `Result`).

6. **`Command::apply_to_model`** — on `Outcome::Ok` only, update balances and
   `total_supply` the same way the contract did. Failures must not mutate the
   shadow state.

7. **Harness registration** — `harness()` wires
   `NoNegativeBalance`, `SupplyConservation`, and `AuthRequiredForMutation`
   from `soro-fuzz-invariants`. Compare with the counter, which registers a
   single local invariant.

8. **Drivers** (unchanged shape from the counter):
   - `examples/token/tests/proptest_mirror.rs`
   - `fuzz/fuzz_targets/token_fuzz.rs`

When your contract is "token-shaped" (balances + supply + auth), copy the
token harness. When it is a tiny state machine with one novel rule, the
counter's inline invariant is still the right template.
